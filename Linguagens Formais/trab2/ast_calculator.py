'''
# Membros do grupo
Gabriel Trindade	120019606
Jhayson Jales		120059038
Riquelme Gomes		120032785
'''
...
class Parser:
    ...
    def parseVS(self):
        ...
        # Realiza o parsing de atribuições subsequentes caso existam
    ...
    def parsePS(self):
        ...
        # Realiza o parsing de cálculos subsequentes caso existam
        while self.peek('NEWLINE'):
            self.consume('NEWLINE')
            if self.peek('ARROBA'):
                self.consume('ARROBA')
...
# Função exibe a lista dos tokens no terminal
def printTokens(tokens):
    # Exibe os tokens lidos no terminal
    for token in tokens:
        print(token.tokenType, token.tokenValue)

# Função calcula a expressão formada pela árvore sintática
...
if __name__ == '__main__':
    # Recebe o path do arquivo .txt a ser analisado pelo terminal na chamada de execução do programa
    #file_path = './entrada.txt'
    #'''
    if len(argv) != 2:
        sys.exit("Insira path do arquivo no terminal no formato: <path/para/python.exe> <path/para/programa.py> <path/para/arquivo.txt>")
    ...
    syntaxTree = parser.parseS()

    # executa o programa, calculando esxpressão da árvore sintática
    valueOfVariables = {}
    calculateExpression(syntaxTree, valueOfVariables)
